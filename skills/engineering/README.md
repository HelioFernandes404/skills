...
- **[argocd](./argocd/SKILL.md)** — ArgoCD REST API skill for GitOps automation via HTTP/curl: create/sync apps, ApplicationSet specs, bearer token auth, sync options, resource hooks.
- **[argocd-cli](./argocd-cli/SKILL.md)** — ArgoCD CLI skill for GitOps automation via the `argocd` command: login, apps, appsets, projects, repos, clusters, accounts.
